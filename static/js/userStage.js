...
window.onload = async () => {
  if (localStorage.getItem("gameEnabled") !== "userstage"
  ...
  const mapinfo = JSON.parse(
    JSON.parse(
      await fetchJSON("api/create/play", {
        "stageid": localStorage.getItem("userstageid")
      })
    )
  );
  console.log(mapinfo)
  ...
  modal.addEventListener("click", async () => {
    ...
    await fetchJSON("api/create/clear", {
      "userid": localStorage.getItem("userid"),
      "stageid": localStorage.getItem("userstageid"),
